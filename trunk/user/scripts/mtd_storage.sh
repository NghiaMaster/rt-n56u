...
func_fill()
{
	...
	script_calldata="$dir_storage/calldata.sh"
...
EOF
	...
	# create EC200T Call DATA script
	if [ ! -f "$script_calldata" ] ; then
		cat > "$script_calldata" <<EOF
#!/bin/sh

sleep 10
echo -ne "at+qicsgp=1,1,\"internet\"\r\n" | microcom -X -t 2000 /dev/ttyUSB1
sleep 3
echo -ne "at+qnetdevctl=1,1,1\r\n" | microcom -X -t 2000 /dev/ttyUSB1

EOF
		chmod 755 "$script_calldata"
	fi
